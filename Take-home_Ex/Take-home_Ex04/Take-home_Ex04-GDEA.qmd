...
This assignment is separated into three segments (web pages):
    1.  Initial Data Exploratory Analysis (IDEA) – [Click here for IDEA page](Take-home_Ex/Take-home_Ex04/Take-home_Ex04-IDEA.html){target="_blank"}.
    2.  Geospatial Data Exploratory Analysis (GDEA) – Current Page
    3.  Prototype: Exploratory – [Click here for Prototype: Exploratory page.](Take-home_Ex/Take-home_Ex04/Take-home_Ex04-Prototype.html){target="_blank"}
...
pacman::p_load(tidyverse, kableExtra, knitr,
               ...
               leaflet.extras, mapview, leafsync, # hotspot
               tm, plotly)
```
...
Use st_join to combine aspatial data and geospatial data.
...
The code chunk below to compute density area of total armed conflict incidents and total fatalities at each sub-national administrative region level, and filter the selected variables for subsequential geospatial analysis.
...
The flowchart diagram below shows an overview of the four geospatial explorations.
...
Use `boxmap()` to plot boxmap and `tmap_arrange()` to arrange small multiple maps in grid layout.
...
This segment will visualise spatial density (concentration) within the different geographical subnational administrative regions by plotting density map.
...
-   The prototype proposes to include the following interactivity elements for users' geospatial data exploratory:
    ...
    -   Dropdown selection options on classification types and number of classes
:::

### Visualising Spatial Density of Armed Conflicts

The code chunk below uses `tm_shape()` to plot the density map and density variables created earlier.
...
-   The prototype proposes to include the following interactivity elements for users' geospatial data exploratory:
    -   Radio button selection on *total armed conflicts* or *total fatalities*
    -   Checkbox selection to filter/ select by *event_type*
    -   Slider bar to select the *years*
-   Information such as event date, country, sub-national administrative region (1, 2, 3), event type, summary of incident, and number of fatalities of each armed conflict incident.
:::
...
Prototype: Exploratory can be found via [link here](Take-home_Ex/Take-home_Ex04/Take-home_Ex04-Prototype.html){target="_blank"}.
...
-   [Basic Mapping, Luc Anselin and Grant Morrison (2018)](https://spatialanalysis.github.io/lab_tutorials/4_R_Mapping.html)
...
-   [ISSS624 AY2022-23 November Term (2023)](https://isss624-ay2022-23nov.netlify.app/)
